...
module CSA.OptWrappedParameterizedOnChain where
...
instance Scripts.ValidatorTypes Benef where
    type instance RedeemerType Benef = BuiltinData  -- Optimized
    -- type instance RedeemerType Benef = Redeem
    -- type instance DatumType Benef = Dat
    type instance DatumType Benef = BuiltinData     -- Optimized
...
-- below benefValidatorWrapped is the optimization
benefValidatorWrapped :: BeneParam -> BuiltinData -> BuiltinData-> Contexts.ScriptContext -> Bool
benefValidatorWrapped benefp d r context = benefValidator benefp (PlutusTx.unsafeFromBuiltinData d) (PlutusTx.unsafeFromBuiltinData r) context

benefCompile :: BeneParam -> Scripts.TypedValidator Benef 
benefCompile benefp = Scripts.mkTypedValidator @Benef 
    ($$(PlutusTx.compile [|| benefValidatorWrapped ||]) `PlutusTx.applyCode` PlutusTx.liftCode benefp)
    $$(PlutusTx.compile [|| wrap ||]) where 
        wrap = Scripts.mkUntypedValidator @BuiltinData @BuiltinData
