...
module CSA.OptParameterizedOnChain where
...
benefValidator :: BeneParam -> BuiltinData -> Redeem -> Contexts.ScriptContext -> Bool
benefValidator benefp d' r context =                       -- Optimized - change `d` to `d'`
-- benefValidator benefp d r context = 
    ...
    where 
        d :: Dat                                         -- Optimization
--        d = (unsafeToBuiltinData d')                     -- Optimization
        d = (PlutusTx.unsafeFromBuiltinData d')                     -- Optimization
...
benefCompile benefp = Scripts.mkTypedValidator @Benef 
    ...
    $$(PlutusTx.compile [|| wrap ||]) where 
        wrap = Scripts.mkUntypedValidator @BuiltinData @Redeem
